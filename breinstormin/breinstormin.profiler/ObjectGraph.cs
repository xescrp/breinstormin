...
namespace breinstormin.profiler
{
    public class ObjectGraph
    {
        ...
        internal void AssignInterestLevelsToTypes(BuildTypeGraphOptions options)
        {
        ...
        internal void AssignInterestLevelToObject(ulong id, GcObject gcObject, BuildTypeGraphOptions options, 
             bool isRoot)
        {
        ...
        internal double TickIndexToTime(int tickIndex)
        {
            if (tickIndex < 0)
                tickIndex = 0;
            if (tickIndex > CLRProfiler.instance.log.maxTickIndex)
                tickIndex = CLRProfiler.instance.log.maxTickIndex;
            return CLRProfiler.instance.log.TickIndexToTime(tickIndex);
        }
        ...
        internal Graph BuildTypeGraph(int allocatedAfterTickIndex, int allocatedBeforeTickIndex, BuildTypeGraphOptions options)
        {
            ...
            AssignInterestLevelsToTypes(options);
            for (int i = 0; i < rootCount; i++)
            {
                AssignInterestLevelToObject(rootIDs[i], roots[i], options, true);
            }
            ...
            foreach (KeyValuePair<ulong, GcObject> keyValuePair in idToObject)
            {
                ...
                if (gcObject.AllocTickIndex > allocatedAfterTickIndex && gcObject.AllocTickIndex < allocatedBeforeTickIndex)
                    AssignInterestLevelToObject(keyValuePair.Key, gcObject, options, false);
                else
                    gcObject.InterestLevel = InterestLevel.Ignore;
